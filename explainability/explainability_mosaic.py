...
from consts.consts import ArchArgs, XmethodArgs, MosaicArgs
...
def main(args):
    ...
    for mosaic_filepath, target_class, images_filenames, image_labels in zip(*dataset.get_subset()):
        ...
        if not os.path.exists(output_path):
            mosaic_explanation = explaining_method.explain(mosaic_filepath, target_class, baseline=args.baseline)
...
def get_args():
    ...
    parser.add_argument('-baseline', type=str, help='Baseline', default=None, choices=['black', 'uniform', 'blurred', 'gaussian'])
